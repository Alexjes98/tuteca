...
func _process_movement(delta: float) -> void:
	...
	if climb_held and not is_on_floor():
		...
		# Keeping the last good normal valid during brief gaps caused by
		# rotate_y() breaking contact mid-frame (camera mouse look).
		if is_on_wall():
			_climbing    = true
			_wall_normal = get_wall_normal()
		# else: remain in whatever _climbing state was — avoids false drops.
	else:
